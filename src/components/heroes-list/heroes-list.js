...
class HeroesList extends React.Component {
  ...
  componentDidMount() {
    const { fillWithHeroes } = this.props;
    this
      .marvelHeroes
      .getAllCharacters()
      .then(body => fillWithHeroes(body));
  }
  ...
  render() {
    ...
    let filteredHeroes = [];
    if (heroes.length > 0) {
      filteredHeroes = heroes.filter(hero => hero.name.toLowerCase()
...
HeroesList.propTypes = {
  ...
  fillWithHeroes: PropTypes.func.isRequired,
